...
*/
    ...
    const vk_verifier = unstringifyBigInts$2(_vk_verifier);
...
async function wtnsCheck(r1csFilename, wtnsFilename, logger) {
    ...
    const wtnsHeader = await readHeader$1(fdWtns, wtnsSections);
...
async function exportFFlonkVk(zkey, logger) {
    ...
    return stringifyBigInts$1(vKey);
}
...
const {unstringifyBigInts: unstringifyBigInts$5, stringifyBigInts: stringifyBigInts$2} = ffjavascript.utils;
...
async function fflonkExportSolidityVerifier(vk, templates, logger) {
    ...
    function fromVkey(str) {
        const val = unstringifyBigInts$5(str);
...
async function plonk16Prove(zkeyFileName, witnessFileName, logger) {
    ...
    const zkey = await readHeader(fdZKey, zkeySections);
    ...
    return {
        proof: stringifyBigInts$3(_proof),
...
async function plonkVerify(_vk_verifier, _publicSignals, _proof, logger) {
    let vk_verifier = unstringifyBigInts$7(_vk_verifier);
    _proof = unstringifyBigInts$7(_proof);
    let publicSignals = unstringifyBigInts$7(_publicSignals);
    ...
    if (!isWellConstructed$1(curve, proof)) {
...
async function fflonkProve(zkeyFileName, witnessFileName, logger) {
    ...
    const wtns = await readHeader$1(fdWtns, wtnsSections);
...
async function fflonkVerify(_vk_verifier, _publicSignals, _proof, logger) {
    ...
    _vk_verifier = unstringifyBigInts$a(_vk_verifier);
    _proof = unstringifyBigInts$a(_proof);
    ...
    const pi = calculatePI$1(curve, publicSignals, lagrangeEvals);
...
function calculatePI$1(curve, publicSignals, lagrangeEvals) {
    const Fr = curve.Fr;
...
async function fflonkExportCallData(_pub, _proof) {
    ...
    return `[${p256$2(proof.polynomials.C1[0])}, ${p256$2(proof.polynomials.C1[1])},` +
    `${p256$2(proof.polynomials.C2[0])},${p256$2(proof.polynomials.C2[1])},` +
    `${p256$2(proof.polynomials.W1[0])},${p256$2(proof.polynomials.W1[1])},` +
    `${p256$2(proof.polynomials.W2[0])},${p256$2(proof.polynomials.W2[1])},` +
    `${p256$2(proof.evaluations.ql)},${p256$2(proof.evaluations.qr)},${p256$2(proof.evaluations.qm)},` +
    `${p256$2(proof.evaluations.qo)},${p256$2(proof.evaluations.qc)},${p256$2(proof.evaluations.s1)},` +
    `${p256$2(proof.evaluations.s2)},${p256$2(proof.evaluations.s3)},${p256$2(proof.evaluations.a)},` +
    `${p256$2(proof.evaluations.b)},${p256$2(proof.evaluations.c)},${p256$2(proof.evaluations.z)},` +
    `${p256$2(proof.evaluations.zw)},${p256$2(proof.evaluations.t1w)},${p256$2(proof.evaluations.t2w)},` +
    `${p256$2(proof.evaluations.inv)}],` +
    `[${inputs}]`;
}
