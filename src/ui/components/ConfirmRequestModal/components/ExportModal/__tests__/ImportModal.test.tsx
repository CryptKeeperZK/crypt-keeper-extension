...
import { ExportModal, ExportModalProps } from "..";
import { IUseProofModalData, useExportModal } from "../useExportModal";

jest.mock("../useExportModal", (): unknown => ({
  useExportModal: jest.fn(),
}));
...
describe("ui/components/ConfirmRequestModal/components/ProofModal", () => {
  ...
  beforeEach(() => {
    (useExportModal as jest.Mock).mockReturnValue(defaultHookData);
  ...
  test("should render properly with error", async () => {
    render(<ExportModal {...defaultProps} error="Error" len={2} />);
